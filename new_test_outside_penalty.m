...
%                           u=sqrt(abs(SIT_T2(opt_param(k,:)))*abs(SIT_T3(opt_param(k,:))));
%                           opt_fval(k)=abs(opt_fval(k))-u;
...
%                 opt_fval(1:end-1)=abs(opt_fval(1:end-1))-u;
                ...
                [real_opt_fval,real_opt_fval_idx]=max((opt_fval));
            ...
            for i=1:iter
                ...
                if(abs(test_baba(i))<num^(3/4)*num^(-3/8))
                    test_baba_opt(i) = test_baba(i);
                    test_opt_param(i,:) = [sample_m,1-eps];
                else
                % SIT Test result
